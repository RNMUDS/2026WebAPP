...
- マルチプレイできる自由な3D空間を実装する（複数人が入りアバタが同期）【必須】
- 入室者の行動データ（入退室・移動・クリックなど）を SQLite に記録する【必須】
- 集めたデータをダッシュボードで集計・閲覧できるようにする【任意】
- デバッグ手法を活用して問題を解決できる

> 動く参考実装がリポジトリの `Web14/` にある（`db.js` / `server.js` / `public/index.html` / `public/dashboard.html`）。
> `npm install && node server.js` で起動し、コピー＆改造の土台にする。
> 第15回では起動時に表示されるIPアドレスをクラスメイトに伝えて入室してもらい、実際にデータを取得する。
